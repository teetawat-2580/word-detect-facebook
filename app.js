...
(function () {
  ...
  async function init() {
    ...
    // Try loading real_posts.json automatically if generated by auto_collector.py
    let loadedReal = await tryLoadRealPostsJson();

    if (!loadedReal) {
      // Check localStorage for auto-extracted posts
    ...
    if (!loadedReal && window.SAMPLE_FB_GROUP_DATA && window.SAMPLE_FB_GROUP_DATA.posts) {
  ...
  async function tryLoadRealPostsJson() {
    try {
      ...
      // Ignore if real_posts.json doesn't exist yet
    }
    return false;
  }
    ...
    if (state.filteredPosts.length === 0) {
      DOM.postsFeed.innerHTML = `
        <div class="empty-state">
          ...
          <p class="empty-desc">ลองรัน auto_collector.py หรือลากไฟล์ HTML มาวางเพื่อนำเข้าโพสต์จริง</p>
